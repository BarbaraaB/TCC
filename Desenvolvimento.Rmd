...
tdm <- as.matrix(tdm)
tdm_c <- tdm[nchar(rownames(tdm)) < 11,]
...
```{r}
#line plot
ggplot(sentiment_month_longer) + aes(x=mes, y=value, color = name, group=name) + geom_line() + geom_point()
ggplot(pos_neg_month_longer) + aes(x=mes, y=value, color = name, group=name) + geom_line() + geom_point()

wordcloud(words = d$word, freq = d$freq, max.words = 250, random.order = FALSE)
comparison.cloud(tdm_c, random.order=FALSE, max.words=10)
